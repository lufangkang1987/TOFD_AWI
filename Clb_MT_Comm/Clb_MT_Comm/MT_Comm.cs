...
namespace Clb_MT_Comm
{
    public class MT_Comm
    {
        ...
        /// <summary>
        /// 光栅臂自动模式下是否纠偏：01：左纠偏   02：右纠偏  03：停止纠偏
        /// </summary>
        public     int m_i_Jp = 3;
        /// <summary>
        /// 处理完再接收数据
        /// </summary>
        public bool m_bl_GetCom_Data = true;
        /// <summary>
        /// 接收数据
        /// </summary>
        Thread Tread_GetCom_Data;
        /// <summary>
        /// 消息接口
        /// </summary>
        ...
        /// 等待探头下压到位时间
        /// </summary>
        public int m_i_Down_WaitTime = 2;
        /// <summary>
        /// 打开设备
        /// </summary>
        UInt32 m_bOpen = 0;
        ...
        /// 光栅臂位置
        /// </summary>
        public int m_Para_i_GsbWz_mm = 0;
        /// <summary>
        /// 车体位置
        /// </summary>
        public int m_Para_i_Dist_mm = 0;
        /// <summary>
        /// 光栅臂移动方向，true：左往右  false: 右往左
        /// </summary>
        public bool blGsb_RunFx_L_to_R = false;
        /// <summary>
        /// 拍照：1 录像开始：2 录像结束：3
        /// </summary>
        public int m_Para_i_Photo1_Vb2_Ve3 = 0;
        ...
        /// 光栅臂距离
        /// </summary>
        public string strLogFileName_Gsb = Application.StartupPath + "\\datalog\\Gsb_Log.ini";
        /// <summary>
        /// 读写文件
        /// </summary>
        ...
        public string strBtl = "38400,0,8,1";
        #endregion
 ...
 */
        ...
        private void Par_ComData(object ArrGet)
        {
            byte[] _btArrGet = (byte[])ArrGet;


            m_bl_GetCom_Data = false;
            ...
            string _strCmdRev_Para = SerialClass.byteToHexStr(_btArrGet);//可能包含帧头帧尾的接收报文转换字符串
            ...
            while (_strCmdRev_Para.Length > 15)
            {
                _blHave_Para = false;
                {
                    ...
                    strCmdRev = SerialClass.byteToHexStr(_btArrGet);//解析报文
                    if (strCmdRev.Length == 0) continue;
                    m_blLink = true;
                    #region   1、动作指令
             ...
             第八字节 01：左纠偏
                  ...
                  第八字节：右限位参数低位
                    ...
                    if (_btArrGet[0] == 0x4D && _btArrGet[1] == 0x53)
                    {
                 ...
                 第八字节：滑台参数低位
                    ...
                    if (_btArrGet[0] == 0x43 && _btArrGet[1] == 0x59)
                    {
                        ...
                        if (m_SysBuf.iBmq_Type == 1)
                        {
                            float _flDis = (float)(_dbJuli * m_SysBuf.fl4Car_JzXs);
                            m_SysBuf.Trip_Com_mm = (int)_flDis;
                            _flDis /= 1000;//m
                            m_SysBuf.Trip = _flDis;
                        }
                        else
                        {
                           m_SysBuf.Trip_Com_mm = (int)_dbJuli;

                            m_SysBuf.Trip = _dbJuli / 1000f;//2023-1-11 add
                        }              //double  _flInd =double .Parse ( (Math.Abs(_dbJuli) / m_MainBuff.g_Climb.Interval).ToString ("f0"));
                        m_Para_i_Dist_mm = m_SysBuf.Trip_Com_mm;
                 ...
                 第五六七八字节分别是总行程的高位至低位(因为单位是mm所以参数需要使用四个字节来表达一个参数)
                    ...
                    if (_btArrGet[0] == 0x4F && _btArrGet[1] == 0x53)
                    {
                        ...
                        if (m_SysBuf.m_i_TOFD_0_Cscan_1_Mui_2 == 3 && m_SysBuf.m_bl_Have==false  )
                            m_SysBuf.i_Gsb_Interval = m_SysBuf.i_Para_Dbjg;
                 ...
                 第八字节：纠偏参数低位
                    ...
                    */
                    if (_btArrGet[0] == 0x43 && _btArrGet[1] == 0x58)
                    {
                        ...
                        m_iT = Convert.ToInt32(strCmdRev.Substring(8, 8), 16);
                        ...
                        blGsb_RunFx_L_to_R = m_SysBuf.blGsb_RunFx_L_to_R;
                  ...
                  //      m_SysBuf.strTrip_mm += ((m_SysBuf.strTrip_mm == "" ? "" : ",") + m_SysBuf.i_Para_Wz.ToString());
                     ...
                     //   m_SysBuf.i_Para_Wz = m_iT;
                        ...
                        if (m_SysBuf.m_i_TOFD_0_Cscan_1_Mui_2 != 3)
                        {
                            if (m_SysBuf.blGsb_RunFx_L_to_R)//光栅臂   从左往右
                            {
                                //1 开始测量时延时接收数据
                                flDistance_Y -= m_SysBuf.m_UI_Gsb_Distan_Min;
                                //   flDistance_Y -= m_SysBuf.i_Gsb_Start_Pos;
                                //if (flDistance_Y <= m_SysBuf.i_Gsb_Start_Pos)
                                //{
                                //    flDistance_Y = m_SysBuf.i_Gsb_Start_Pos;// return;
                                //}
                            }
                            else //光栅臂   从右往左
                            {
                                ...
                                //if (flDistance_Y >= m_SysBuf.i_Gsb_End_Pos )
                                //{
                                //    flDistance_Y = m_SysBuf.i_Gsb_End_Pos; //return;
                                //}
                            }
                            if (flDistance_Y >= m_SysBuf.i_Gsb_End_Pos)// m_SysBuf.iGsb_Len)
                            {
                                flDistance_Y = m_SysBuf.i_Gsb_End_Pos - 1; //goto JumpPosition; ;
                            }
                            if (flDistance_Y <= m_SysBuf.i_Gsb_Start_Pos)
                            {
                                flDistance_Y = m_SysBuf.i_Gsb_Start_Pos;// goto JumpPosition; ;
                            }
                        }
                        m_Para_i_GsbWz_mm=(int)(flDistance_Y);
                        m_SysBuf.i_Para_Wz = m_Para_i_GsbWz_mm;
                        #endregion 超声延时数据处理

                        _blHave_Para = true;

                        _strCmd_Type = "光栅臂位置:" + m_iT.ToString() + "   " + flDistance_Y;
                        //       goto JumpPosition;
                  ...
                  第五六七八字节分别是滑台数据的高位至低位
                    ...
                    if (_btArrGet[0] == 0x4D && _btArrGet[1] == 0x4B)
                    {
                        m_SysBuf.flAnle = _btArrGet[7];
                        switch (_btArrGet[7])
                        {
                            case 24://车体上传打标笔是否安装）未安装
                                _strCmd_Type = "(车体上传打标笔芯是否耗尽）未耗尽";
                                _blHave_Para = true;
                                break;
                            case 25:
                                _strCmd_Type = "(车体上传打标笔是否安装）已安装";
                                _blHave_Para = true;
                                break;//
                            case 27:
                                _strCmd_Type = "(车体上传打标笔芯是否耗尽）未耗尽";
                                ...
                                break;//
                            case 28:
                                _strCmd_Type = "(车体上传打标笔芯是否耗尽）已耗尽";
                                _blHave_Para = true;
                                break;//
                            case 1://收到打标动作
                                m_SysBuf.bl_Discharge_To_Mark = true;
                                m_SysBuf.bl_Discharge_Show_To_Mark = true;
                             
                                m_SysBuf.bl_Discharge_To_Mark_Txt = true;
                                m_SysBuf.i_Discharge_To_Mark_Txt_X = m_SysBuf.Trip_Com_mm;
                                m_SysBuf.i_Discharge_To_Mark_Txt_Y  = m_SysBuf.i_Para_Wz;
                                m_SysBuf.str_Discharge_To_Mark_Txt_Time = DateTime.Now.ToString("HH:mm:ss:fff");
                                break;
                        }
                    }
                    if (_btArrGet[0] == 0x53 && _btArrGet[1] == 0x44)
                    {
                        m_SysBuf.bl_Discharge = true;
                        m_SysBuf.bl_Discharge_Show = true;

                        m_SysBuf.bl_Discharge_Txt = true;
                        m_SysBuf.i_Discharge_Txt_X = m_SysBuf.Trip_Com_mm;
                        m_SysBuf.i_Discharge_Txt_Y  = m_SysBuf.i_Para_Wz;
                        m_SysBuf.str_Discharge_Txt_Time = DateTime.Now.ToString("HH:mm:ss:fff");
                    }
                    if (_btArrGet[0] == 0x55 && _btArrGet[1] == 0x44)
                    {
                        if (_btArrGet[7]==17)
                        m_SysBuf.iUpDownState = 17;
                    }
                                #endregion  附加指令
            ...
            // JumpPosition:
                  WriteErrorLog(strLogFileName, (_strCmd_Type + " 报文：") + _strCmdRev_Para_Old, false);

            m_bl_GetCom_Data = true;
        }
        /// <summary>
        /// 数据解析
        /// </summary>
        ...
        public void ParseDat(object sender, SerialDataReceivedEventArgs e, byte[] _btArrGet)
        {
         //   if (m_bl_GetCom_Data == false) return;
           
            Tread_GetCom_Data = new Thread(new ParameterizedThreadStart(Par_ComData));
            Tread_GetCom_Data.Start((object)_btArrGet);
        ...
        public void ParseDat_Can(byte[] _btArrGet) 
        {
         ...
         第八字节 01：左纠偏
             ...
             第八字节：纠偏参数低位
                ...
                */
                if (_btArrGet[0] == 0x43 && _btArrGet[1] == 0x58)
                {
                    ...
                    m_SysBuf.blGsb_RunFx_L_to_R = Convert.ToInt32(strCmdRev.Substring(4, 2), 16) == 1;// m_SysBuf.blGsb_RunFx_L_to_R = m_bl_L_R;//1:远离原点 2：向原点运动
                    m_SysBuf.strTrip_mm += ((m_SysBuf.strTrip_mm == "" ? "" : ",") + m_SysBuf.i_Para_Wz.ToString());
                    if (m_SysBuf.blGsb_RunFx_L_to_R_Old != m_SysBuf.blGsb_RunFx_L_to_R)
                    {
                        m_iGsb_No = 0;
                        if (m_SysBuf.strTrip_mm.Length > 20)
                            WriteError_Gsb_Log(strLogFileName_Gsb, false, (m_SysBuf.Trip_Com_mm - m_SysBuf.i_Gsb_Interval).ToString() + "--" + m_SysBuf.strTrip_mm);
                    ...
                    {   //当前光栅臂位置
                        ...
                        if (flDistance_Y <= m_SysBuf.i_Gsb_Start_Pos)
                        {
                            flDistance_Y = m_SysBuf.i_Gsb_Start_Pos;// + 1;// goto JumpPosition; ;
                        }
                    }
                    m_SysBuf.i_Para_Wz = (int)(flDistance_Y);
                    #endregion 超声延时数据处理
                    _strCmd_Type += "  " + flDistance_Y;
                    goto JumpPosition;
            ...
            WriteErrorLog(strLogFileName, (_strCmd_Type + " 报文："), false);
          //  int _11;
        }
        ...
        {
            string strEnd = "00000D";
            string strSendData = "FE10";
            string Cmd_SendObjecAddr = "";
            string Cmd_SendSourceAddr = "";// 6,36,1-n
            string strCmd = iComType_1.ToString ();//36
            string Cmd_SendInfo = iIndex.ToString();//1-6
                        ...
                        case 1://1、动作指令
                            ...
                            switch (iIndex)//字节位置
                            {
                                case 3:
                                    switch (int.Parse(strVal))
                                    {
                                        ...
                                        case 3://后退
                                            ...
                                            break;
                                        case 4:
                                            _btArrSend[--iIndex] = 0x04;//自动模式启动(前进)(锯齿扫查)
                                            break;
                                        case 5:
                                            _btArrSend[--iIndex] = 0x05;//自动模式启动(后退)(锯齿扫查)
                                            break;
                                        
                                        case 6://前进
                                            _btArrSend[--iIndex] = 0x06;// 直行 光栅臂到达指定位置的前进自动模式启动 2021-04-30增加
                                            break;
                                        case 7://后退
                                            _btArrSend[--iIndex] = 0x07;//直行 光栅臂到达指定位置的后退自动模式启动 2021-04-30增加
                                            break;
                                        case 10://10:自动模式暂停(车体自己和遥控型号都能让车暂停)
                                            _btArrSend[--iIndex] = 0x0A;
                                            break;
                                        case 11://自动模式(暂停后)继续
                                            _btArrSend[--iIndex] = 0x0B;
                                            break;


                                        //2024-11-11添加
                                        case 14://前进
                                            _btArrSend[--iIndex] = 0x0E;//涂层和超声测厚 自动模式前进启动
                                            break;
                                        case 15://后退
                                            _btArrSend[--iIndex] = 0x0F;//涂层和超声测厚 自动模式后退启动 2021-03-10增加
                                            break;

                                        case 16://前进
                                            _btArrSend[--iIndex] = 0x10;//涂层 自动模式前进启动
                                            break;
                                        case 17://后退
                                            _btArrSend[--iIndex] = 0x11;//涂层 自动模式后退启动 2021-03-10增加
                                            break;
                                ...
                                case 8://第八字节 01：左纠偏
                                    ...
                                    if (_iT > 0 && _iT < 4)
                                        _btArrSend[--iIndex] = (byte)_iT;
                                    else if(_iT==17)//11：视觉寻迹行走打开
                                        _btArrSend[--iIndex] = (byte)_iT;
                                    else if (_iT == 18)//12：视觉寻迹行走关闭
                                        _btArrSend[--iIndex] = (byte)_iT;
                                    break;
                            }
                ...
                case 2://二、参数指令
                    ...
                    switch (iComType_1)
                    {
                        case 1://1、滑台参数
                            ...
                            {
                                if (iRw == 0)
                                    _btArrSend[3] = 0x10;// 10：向打磨机写入参数 
                                else
                                    _btArrSend[3] = 0x11;//11：向打磨机读取参数
                            }

                            _sPara = strVal.Split(',');//车体速度参数,滑台参数
                            if (_sPara.Length == 2)
                            {
                   //             m_MainBuff.g_Climb.m_fl_Gsb_Start = int.Parse(_sPara[0]) / 1000f;
                   //             m_MainBuff.g_Climb.m_fl_Gsb_End = int.Parse(_sPara[1]) / 1000f;
                        ...
                        case 4://4、车体单次行程参数
                            ...
                            //---------
                            _sPara = strVal.Split(',');//光栅臂间隔，车体前进间隔
                            if (_sPara.Length == 2)
                            {
                                string _str16 = Convert.ToString(int.Parse(_sPara[1]), 16);
                                _str16 = strAddLen_Num(_str16);
                                _btArrSend[4] = Convert.ToByte(_str16.Substring(0, 2), 16); //第五字节:光栅臂间隔参数高位
                                _btArrSend[5] = Convert.ToByte(_str16.Substring(2, 2), 16); ;//第六字节；光栅臂间隔参数高位

                                _str16 = Convert.ToString(int.Parse(_sPara[0]), 16);
                                _str16 = strAddLen_Num(_str16);
                                _btArrSend[6] = Convert.ToByte(_str16.Substring(0, 2), 16);//第七字节：单步参数高位
                                _btArrSend[7] = Convert.ToByte(_str16.Substring(2, 2), 16);//第八字节：单步参数高位
                            }
                            //-------
                            //strVal = Convert.ToString(int.Parse(strVal), 16);
                ...
                case 4://四、磁粉项目增加指令
                    ...
                    switch (iComType_1)
                    {
                        ...
                        case 2://二、手动速度参数
                            ...
                            strVal = strAddLen_Num(strVal);
                            _btArrSend[4] = Convert.ToByte(strVal.Substring(0, 2), 16); //第5字节：车体自动速度高位
                            _btArrSend[5] = Convert.ToByte(strVal.Substring(2, 2), 16); //第6字节：车体自动速度低位
                           
                            strVal = Convert.ToString(int.Parse(_sPara[1]), 16);
                            strVal = strAddLen_Num(strVal);
                            _btArrSend[6] = Convert.ToByte(strVal.Substring(0, 2), 16); //第七字节：滑台自动速度高位
                            _btArrSend[7] = Convert.ToByte(strVal.Substring(2, 2), 16); //第八字节：滑台自动速度低位
                ...
                case 5://五、TOFD磁爬车增加指令
                    ...
                    switch (iComType_1)
                    {
                        ...
                        前两字节是指令的针头，用于识别指令的类型；	 
                            ...
                            if (int.Parse(strVal) == 1)//2023-11-1 将此值改反改成0，原因是硬件反了
                    ...
                    break;
                case 6://新打标：从2轮移植过来
                    Cmd_SendObjecAddr = "0106";//车体设备命令集  目标地址0106  源地址0103
                    Cmd_SendSourceAddr = "0103";

                    strSendData += Cmd_SendObjecAddr + Cmd_SendSourceAddr +
                                          strCmd.ToUpper() + Cmd_SendInfo + strEnd;
                    //switch (iInfo)
                    //{
                    //    case 1:// "打标(自动完成一次打标动作)"
                    //    case 2:// "打标机构降"
                    //    case 3://"打标机构升"
                    //    case 4://"打标机构停止升降"
                    //    case 11://"打标笔芯旋出"
                    ...
                    //27  未耗尽
                    //28  已经耗尽
                    //    case 31:// "直行一次(车体自动识别打标笔是否安装)";
                    //   32 笔芯旋出长度自动识别  关闭
                    //   33 笔芯旋出长度自动识别  开启
                    //        Cmd_SendInfo = iInfo.ToString("00");
                    //        break;
                    //}
                    _btArrSend = Encoding.Default.GetBytes(strSendData);
                    break;
   ...
   //         MessageBox .Show (_strSend );
        ...
        /// <param name="blSendOrRec">true：发送 false:返回</param>       
        /// <param name="strMsg">写入信息</param>
        public void WriteError_Gsb_Log(string strPathFileName, bool blSendOrRec, string strMsg)
        {
            bool _blW=  m_csInter.IniReadDefine("CmmClimb", "GSB_LOG", "0", Application.StartupPath + "\\database\\SysConfig.ini") == "1";
            if (_blW == false) return;
